...
const Navbar = () => {
  ...
  className={`
    ...
    max-md:justify-center gap-6
    md:px-8 py-3
    max-md:h-screen md:rounded-full
    backdrop-blur bg-black/10 md:bg-white/10
...
>
      ...
      <div className="flex items-center gap-8">
        ...
        {
          !user? (  <button className="px-4 py-1 sm:px-7 sm:py-2 bg-primary hover:bg-primary-dull transition rounded-full font-medium cursor-pointer" onClick={openSignIn}>
          {" "}
          Login
        </button>):(
          <UserButton/>
      ...
      <MenuIcon className="w-8 h-8 cursor-pointer md:hidden" onClick={()=>setIsOpen(!isOpen)} />
    </div>
  );
};
